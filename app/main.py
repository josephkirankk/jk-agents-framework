...
def load_app_config(cfg_path: Path | None = None) -> AppConfig:
    ...
    try:
        ...
        # This is needed because AppConfig doesn't have a 'memory' field that matches
        # the YAML structure (it has 'conversation_memory' instead)
        if "memory" in data:
        ...
        # Preserve raw memory config even in fallback case
        if "memory" in data:
            app_cfg._raw_memory_config = data["memory"]
        return app_cfg
