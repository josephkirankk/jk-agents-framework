...
async def build_agent(
    ...
    *,
    ...
    # Temporarily disable checkpointer for performance testing
    # if checkpointer is None:
    #     checkpointer = get_global_checkpointer(app_config)
    #     log.info(f"Using global checkpointer for agent {agent_cfg.name}")
    checkpointer = None
    log.info(f"Checkpointer disabled for performance testing - agent {agent_cfg.name}")
    ...
    if agent_type == "normal":
        ...
        if large_data_config and large_data_config.get("enabled", False):
            ...
            log.info(f"Creating react agent {agent_cfg.name} (standard)")
            agent = create_react_agent(
                model=model_with_tools,
                tools=tools,
                prompt=prompt_filled,
