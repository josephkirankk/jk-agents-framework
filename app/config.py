...
class MCPServerConfig(BaseModel):
    description: Optional[str] = Field("", description="Short description of what this MCP server provides")
    ...
    def require_url_for_http(cls, v, values):
        if values.get("transport") in ("streamable_http", "sse", "http") and not v:
...
class AppConfig(BaseModel):
    ...
    business_context: Optional[str] = Field("", description="Optional business context injected into supervisor prompt")
    persistence: Dict[str, str] = Field(default_factory=lambda: {"type": "memory"})
