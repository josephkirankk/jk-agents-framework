#!/usr/bin/env python3
"""
Run the Schema-Agnostic Test Data Generator and verify results

This script:
1. Loads the json_schema_test_data_generator configuration
2. Runs the workflow with the StudentExamRecord schema
3. Monitors execution to ensure agents use run_python_code tool
4. Verifies the database contains 900 records
5. Runs integration tests to validate data quality
"""
...
async def run_test():
    """Run the test data generator workflow"""
    
    print("=" * 80)
    print("  Schema-Agnostic Test Data Generator - Live Test")
    print("=" * 80)
...
Request : {TEST_REQUIREMENTS}"""
    ...
    # Step 3: Build supervisor and agents
    print("\n[Step 3] Building supervisor and agents...")
    ...
    try:
        supervisor = build_supervisor_compiled(
            ...
            original_user_question=user_request,
            config_path=config_path,
            default_temperature=app_config.temperature or 0.2,
            thread_id=thread_id,
        )
        
        print(f"✅ Supervisor built")
        ...
        print(f"❌ Failed to build supervisor/agents: {e}")
        import traceback
        traceback.print_exc()
        return 1
